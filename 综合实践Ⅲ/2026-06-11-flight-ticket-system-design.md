...
| 阶段 | 周 | 任务 | 里程碑 |
|---|---|---|---|
| ① 基础搭建 | 第 1 周 | MySQL 建表、SpringBoot 初始化、Scrapy 模板 | 项目骨架可跑 |
| ② 爬虫攻坚 | 第 2-3 周 | 1 个数据源跑通 + 三关校验 + 清洗 | 数据库 ≥1000 条 |
| ③ API + 前端 | 第 4 周 | 查询/筛选/详情 API + Vue 基础页面 | PC 端可演示 |
| ④ AI 基础 | 第 5 周 | AI 出行建议助手（讯飞星火对接） | 基础 AI 任务完成 |
| ⑤ 进阶 RAG | 第 6 周 | FAISS 索引构建 + 检索 + 报告生成 | 进阶 AI 任务完成 |
| ⑥ 小程序 | 第 7 周 | 微信小程序（核心页 + AI 对话页） | 双端可用 |
| ⑦ 部署 | 第 8 周 | Nginx + HTTPS + 域名 + 监控 | 公网可访问 |
| ⑧ 收尾 | 第 9 周 | 报告撰写 + 查重 + 答辩 PPT + U 盘 | 终版提交 |
